  ...
  while (conv) {
    ...
    for (n in 1:nS) {
      ...
      if(length(ndx) > 1){
        ...
        LogL2 <- LogL2 + sum(log(E_out$alpha[length(E_out$alpha)]+1e-8))
        alpha <- E_out$alpha
        beta <- E_out$beta 
        gamma <- alpha * beta
    ...
    if(i > 1){
      ...
      thr4 <- loglike2[i] - loglike2[i - 1]
      if(abs(thr) < thresh){conv = FALSE}
      print(list(thr3=thr3,thr2 = thr2,thr = thr,thr4=thr4))
    }
    print(list(mu = mu,sA = A,pi = pi))
  }
  return(list(sigma = sigma, mu = mu, pi = pi, A = A, loglik = loglike, logPost = logPopst, loglik2 = loglike2, C0 = solve(C0)))
}
