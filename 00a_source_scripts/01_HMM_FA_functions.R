#######################################################################################
###
### File name : 01_HMM_FA_functions.R
...
M_theta_NLP <-function(P_star, alpha){
  ...
  eval_grad_f <- function(x){
    paste1 <- paste("-coefs[", 1:(K-1), "]/", "x[", 1:(K-1), "]", "+(N-coefs[", 
                    1:(K-1), "])/", "(1-x[", 1:(K-1), "])", sep="", collapse=",")
...
# ************ The Main Program *************************************
FACTOR_ROTATE <- function(Y, lambda0, lambda1, start, K, epsilon, 
                          alpha, PX, approximate, stop, varimax, plot = TRUE){
...
myImagePlot <- function(x, lines){
  ...
  text(0.01, 1:nrow(x), labels = yLabels, cex = .5)
...
FACTOR_ROTATE_CP <- function(Y, lambda0, lambda1, start, K, epsilon, 
                             alpha, PX, approximate, stop, varimax, plot = TRUE){
